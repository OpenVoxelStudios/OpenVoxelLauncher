...
class MCLCore extends EventEmitter {
  ...
  async launch() {
    try {
      ...
      if (!java.run) {
        this.emit('debug', `Couldn't start Minecraft due to: ${java.message}`)
        this.emit('close', 1, `Couldn't start Minecraft due to: ${java.message}`)
        return null
      }
      ...
      if (this.options.installer) {
        ...
        if (!this.options.version.custom && code === 0) {
          this.emit('debug', 'Installer successfully ran, but no custom version was provided')
        }
        this.emit('debug', `Installer closed with code ${code}`)
      }
      ...
      if (!fs.existsSync(mcPath)) {
        this.emit('debug', 'Attempting to download Minecraft version jar')
      ...
      this.emit('debug', `Using ${separator} to separate class paths`)
      ...
      this.emit('debug', 'Attempting to download assets')
      ...
      this.emit('debug', `Launching with arguments ${launchArguments.join(' ')}`)

      return this.startMinecraft(launchArguments)
    } catch (e) {
      this.emit('debug', `Failed to start due to ${e}, closing...`)
      return null
    }
  }
  ...
  printVersion() {
    if (fs.existsSync(path.join(appPath, '..', 'package.json'))) {
      ...
      this.emit('debug', `MCLC version ${version}`)
    } else { this.emit('debug', 'Package JSON not found, skipping MCLC version check.') }
  }

  createRootDirectory() {
    if (!fs.existsSync(this.options.root)) {
      this.emit('debug', 'Attempting to create root folder')
  ...
  async extractPackage() {
    if (this.options.clientPackage) {
      this.emit('debug', `Extracting client package to ${this.options.root}`)
  ...
  async getModifyJson() {
    ...
    if (this.options.forge) {
      ...
      this.emit('debug', 'Detected Forge in options, getting dependencies')
      modifyJson = await this.handler.getForgedWrapped()
    } else if (this.options.version.custom) {
      this.emit('debug', 'Detected custom in options, setting custom version file')
