...
class Handler {
    ...
    return new Promise(resolve => {
      child.exec(`"${java}" -version`, (error, stdout, stderr) => {
        if (error) {
          ...
          this.client.emit('debug', `Using Java version ${stderr.match(/"(.*?)"/).pop()} ${stderr.includes('64-Bit') ? '64-bit' : '32-Bit'}`)
          resolve({
            run: true
          })
        }
    ...
    return new Promise(resolve => {
      ...
      _request.on('response', (data) => {
        if (data.statusCode === 404) {
          this.client.emit('debug', `Failed to download ${url} due to: File not found...`)
      ...
      _request.on('error', async (error) => {
        this.client.emit('debug', `Failed to download asset to ${path.join(directory, name)} due to\n${error}.` +
                    ` Retrying... ${retry}`)
  ...
  checkSum (hash, file) {
    return new Promise((resolve, reject) => {
      checksum.file(file, (err, sum) => {
        if (err) {
          this.client.emit('debug', `Failed to check file hash due to ${err}`)
          resolve(false)
        } else {
          resolve(hash === sum)
        }
      ...
      request.get(manifest, (error, response, body) => {
        ...
        if (!error) {
          if (!fs.existsSync(cache)) {
            ...
            this.client.emit('debug', 'Cache directory created.')
          }
          fs.writeFile(path.join(`${cache}/version_manifest.json`), body, (err) => {
            if (err) return resolve(err)
            this.client.emit('debug', 'Cached version_manifest.json')
          })
        }
            ...
            request.get(parsed.versions[desiredVersion].url, (error, response, body) => {
              ...
              if (!error) {
                fs.writeFile(path.join(`${cache}/${this.options.version.number}.json`), body, (err) => {
                  if (err) return resolve(err)
                  this.client.emit('debug', `Cached ${this.options.version.number}.json`)
                })
              }

              this.client.emit('debug', 'Parsed version from version manifest')
              if (error && (error.code === 'ENOTFOUND')) {
  ...
  async getJar () {
    ...
    return this.client.emit('debug', 'Downloaded version jar and wrote version json')
  }
  ...
  async getAssets () {
    ...
    if (this.isLegacy()) {
      if (fs.existsSync(path.join(assetDirectory, 'legacy'))) {
        this.client.emit('debug', 'The \'legacy\' directory is no longer used as Minecraft looks ' +
      ...
      this.client.emit('debug', `Copying assets over to ${legacyDirectory}`)
    ...
    this.client.emit('debug', 'Downloaded assets')
  }
    ...
    if (!fs.existsSync(nativeDirectory) || !fs.readdirSync(nativeDirectory).length) {
      ...
      await Promise.all(stat.map(async (native) => {
        ...
        try {
          ...
          // All is well.
          require('../../../libs/logger').warn('file', e)
        }
      ...
      this.client.emit('debug', 'Downloaded and extracted natives')
    }

    counter = 0
    this.client.emit('debug', `Set native path to ${nativeDirectory}`)
  ...
  async getForgedWrapped () {
    ...
    if (fs.existsSync(versionPath)) {
      try {
        ...
        if (!json.forgeWrapperVersion || !(json.forgeWrapperVersion === this.options.overrides.fw.version)) {
          this.client.emit('debug', 'Old ForgeWrapper has generated this version JSON, re-generating')
        } else {
        ...
        require('../../../libs/logger').warn('file', e)
        this.client.emit('debug', 'Failed to parse Forge version JSON, re-generating')
      }
    }

    this.client.emit('debug', 'Generating a proper version json, this might take a bit')
    ...
    try {
      ...
      this.client.emit('debug', 'Failed to load json files for ForgeWrapper, using Vanilla instead')
      return null
    }
    ...
    if (this.isModernForge(json)) {
      ...
      await Promise.all(json.libraries.map(async library => {
        ...
        this.baseRequest(downloadLink, (error, response, body) => {
          if (error) {
            this.client.emit('debug', `Failed checking request for ${downloadLink}`)
          } else {
  ...
  async getClasses (classJson) {
    ...
    this.client.emit('debug', 'Collected class paths')
    return libs
  }
  ...
  formatQuickPlay () {
    ...
    if (!keys.includes(type)) {
      this.client.emit('debug', `quickPlay type is not valid. Valid types are: ${keys.join(', ')}`)
      return null
    }
  ...
  async getLaunchOptions (modification) {
    ...
    if (this.options.server) this.client.emit('debug', 'server and port are deprecated launch flags. Use the quickPlay field.')
    ...
    this.client.emit('debug', 'Set launch options')
    return args
  }
  ...
  getMemory () {
    if (!this.options.memory) {
      this.client.emit('debug', 'Memory not set! Setting 1GB as MAX!')
      this.options.memory = {
        min: 512,
        max: 1023
      }
    ...
    if (!isNaN(this.options.memory.max) && !isNaN(this.options.memory.min)) {
      if (this.options.memory.max < this.options.memory.min) {
        this.client.emit('debug', 'MIN memory is higher then MAX! Resetting!')
